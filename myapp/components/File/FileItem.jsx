...
function FileItem({ file }) {
  ...
  const {FireStore,deleteFile} = useFireBaseContext();

  const onDeleteFile = async(file) => {
    try {
      const response = await deleteDoc(doc(FireStore, "Files",file.id.toString()));
      setShowTostMsg('File Deleted!!!');
    } catch (error) {
    ...
    >
      ...
      <div className="grid grid-cols-3 place-content-start">
        ...
        <h2 className="text-[15px]">
          ...
          {(file.size / 1024 ** 2).toFixed(2) + " MB"}
          <svg xmlns="http://www.w3.org/2000/svg" onClick={()=>onDeleteFile(file)}
